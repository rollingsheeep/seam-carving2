...
// Parallelized for the first row and then row by row with synchronization between rows
void compute_dynamic_programming(const Matrix& grad, Matrix& dp) {
    assert(grad.width == dp.width && grad.height == dp.height);

    // Initialize first row (can be parallelized)
    #pragma omp parallel for
    for (int x = 0; x < grad.width; ++x) {
        dp.at(0, x) = grad.at(0, x);
    }

    // Fill the rest of the matrix row by row (data dependency between rows)
    for (int y = 1; y < grad.height; ++y) {
        // Each row can be parallelized once the previous row is complete
        #pragma omp parallel for
        for (int x = 0; x < grad.width; ++x) {
...
// Finding minimum in last row is parallelized
void compute_seam(const Matrix& dp, std::vector<int>& seam) {
    seam.resize(dp.height);
    
    // Find the minimum value in the last row (parallelized reduction)
    int y = dp.height - 1;
    ...
    // Backtrack to find the seam (inherently sequential, can't parallelize)
...
// Parallelized by processing rows in parallel
...
// Parallelized for the first row and then row by row with synchronization
void compute_forward_energy(const Matrix& lum, Matrix& energy) {
    assert(lum.width == energy.width && lum.height == energy.height);
    int w = lum.width;
    int h = lum.height;

    // Initialize the first row to 0 (parallelized)
    #pragma omp parallel for
    for (int x = 0; x < w; ++x) {
        energy.at(0, x) = 0.0f;
    }

    // DP forward energy computation (row by row with synchronization)
    for (int y = 1; y < h; ++y) {
        #pragma omp parallel for
        for (int x = 0; x < w; ++x) {
...
// Parallelized for various computations within the function
void compute_hybrid_energy(const Matrix& lum, Matrix& energy) {
    ...
    // Calculate forward and backward energy in parallel
    ...
    #pragma omp parallel
    {
        float local_min_f = FLT_MAX, local_max_f = -FLT_MAX;
        float local_min_b = FLT_MAX, local_max_b = -FLT_MAX;
        ...
        for (int y = 0; y < lum.height; ++y) {
            for (int x = 0; x < lum.width; ++x) {
                ...
                local_min_f = std::min(local_min_f, f_val);
                local_max_f = std::max(local_max_f, f_val);
                local_min_b = std::min(local_min_b, b_val);
                local_max_b = std::max(local_max_b, b_val);
            }
        }
        
        #pragma omp critical
        {
            min_forward = std::min(min_forward, local_min_f);
            max_forward = std::max(max_forward, local_max_f);
            min_backward = std::min(min_backward, local_min_b);
            max_backward = std::max(max_backward, local_max_b);
        }
    }
    ...
    // Normalization can be parallelized
    #pragma omp parallel for collapse(2)
    for (int y = 0; y < lum.height; ++y) {
        for (int x = 0; x < lum.width; ++x) {
    ...
    // Summing for averages (parallel reduction)
    #pragma omp parallel
    {
        float local_sum_f = 0.0f, local_sum_b = 0.0f;
        
        #pragma omp for collapse(2)
        for (int y = 0; y < lum.height; ++y) {
            for (int x = 0; x < lum.width; ++x) {
                local_sum_f += norm_forward_energy.at(y, x);
                local_sum_b += norm_backward_energy.at(y, x);
            }
        }
        
        #pragma omp atomic
        avg_forward += local_sum_f;
        
        #pragma omp atomic
        avg_backward += local_sum_b;
    }
    ...
    // Computing standard deviations (parallel reduction)
    #pragma omp parallel
    {
        float local_sum_sq_f = 0.0f, local_sum_sq_b = 0.0f;
        ...
        for (int y = 0; y < lum.height; ++y) {
            for (int x = 0; x < lum.width; ++x) {
                ...
                local_sum_sq_f += diff_forward * diff_forward;
                local_sum_sq_b += diff_backward * diff_backward;
            }
        }
        
        #pragma omp atomic
        std_dev_forward += local_sum_sq_f;
        
        #pragma omp atomic
        std_dev_backward += local_sum_sq_b;
    }
    ...
    // Counting high energy pixels (parallel reduction)
    #pragma omp parallel
    {
        int local_high_f = 0, local_high_b = 0;
        
        #pragma omp for collapse(2)
        for (int y = 0; y < lum.height; ++y) {
            for (int x = 0; x < lum.width; ++x) {
                if (norm_forward_energy.at(y, x) > forward_threshold) local_high_f++;
                if (norm_backward_energy.at(y, x) > backward_threshold) local_high_b++;
            }
        }
        
        #pragma omp atomic
        high_energy_forward += local_high_f;
        
        #pragma omp atomic
        high_energy_backward += local_high_b;
    }
    ...
    // Use the chosen energy method (copy can be parallelized)
...
void print_usage(const char* program) {
    ...
    std::cerr << "  --threads: Set number of OpenMP threads (default: system max)\n";
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "Using OpenMP implementation\n";
    ...
    // Default number of threads (use system max)
